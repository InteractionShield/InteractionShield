...
class SmartThingsHelper {
    ...
    List sinkFunctionList = ["send", "sendEvent", "sendNotification", "sendLocationEvent",
                             "sendNotificationEvent", "sendNotificationToContacts", "sendPush", "sendPushMessage",
                             "sendSms", "sendSmsMessage", "httpDelete", "httpGet", "httpHead", "httpPost",
                             "httpPostJson", "httpPut", "httpPutJson", "httpError"]


    SmartThingsHelper(DefaultMutableTreeNode rootDmtn) {
        ...
        // Get all closure expression node
        allClosureList = getAllTnwpNodesList(rootEnumList, "Closure")
        closureTnwpMap = getClosureTnwpMap()
    ...
    Map getInformation() {
        ...
        List toBeDelMethodCallList = ["initialize", "installed", "updated", "uninstalled", "preferences", "section",
                                      "paragraph", "mode", "label", "dynamicPage", "page", "href", "icon"]


//        allMethodCallList.removeAll {
//            it ->
//                getTnwpMethodAsString(it) in toBeDelMethodCallList
//        }
//
//        allMethodCallList.removeAll {
//            it ->
//                getTnwpMethodAsString(it).startsWith("MethodCall - log.")
//        }
...
//            if (methodCallTextStr.startsWith("this.preference") ||
//                    methodCallTextStr.startsWith("this.definition") ||
//                    methodCallTextStr.startsWith("this.section") ||
//                    methodCallTextStr.startsWith("this.subscribe") ||
//                    methodCallTextStr.startsWith("this.unsubscribe") ||
//                    methodCallTextStr.startsWith("this.unschedule") ||
//                    methodCallTextStr.startsWith("this.initial") ||
//                    methodCallTextStr.startsWith("this.mapp") ||
//                    methodCallTextStr.startsWith("this.page") ||
//                    methodCallTextStr.startsWith("this.paragraph") ||
//                    methodCallTextStr.startsWith("log.")) {
//                continue
//            }

            def (String receiverStr, String methodStr, List argumentsList) = methodCallParamMap[methodCallTextStr]

            if (methodStr.equals("definition") && receiverStr.equals("this")) {
                (filenameStr, descriptionStr, categoryStr) = getInformFromDefinition(nodeTnwp)
            } else if (methodStr.equals("input") && receiverStr.equals("this")) {
                String sectionStr = getSectionFromInput(nodeTnwp)
                def (String inputNameStr, String inputTypeStr, String inputTitleStr) = getInformFromInput(nodeTnwp)
                // evohome-connect[IoTBench@IoTBench-test-suite].groovy
                if (inputNameStr == null) {
                    continue
                }
                inputMap[inputNameStr] = [inputTypeStr, inputTitleStr, sectionStr]
            } else {
                def (String nameMethodStr, List condList, List condVarTnwpList) = getPathFromMethodCallToMethod(nodeTnwp)
                if(nameMethodStr==null){
                    continue
                }
...
//                "methodCallMethodNodeVarMap": methodCallMethodNodeVarMap
    ...
    /**
     * Get filename and description from definition node
     * @param nodeTnwp : definition method call node
     * @return filename and description
     */
    List getInformFromDefinition(TreeNodeWithProperties nodeTnwp) {

        List nodeEnumList = nodeTnwp.depthFirstEnumeration().toList()
        List allMapEntryList = getAllTnwpNodesList(nodeEnumList, "MapEntryExpression")

        String filenameStr = getValueFromMapEntryList(allMapEntryList, "name")
        if (filenameStr == null) {
            filenameStr = ""
        }
        String descriptionStr = getValueFromMapEntryList(allMapEntryList, "description")
        if (descriptionStr == null) {
            descriptionStr = ""
        }
        String categoryStr = getValueFromMapEntryList(allMapEntryList, "category")
        if (categoryStr == null) {
            categoryStr = ""
        }

        return [filenameStr, descriptionStr, categoryStr]
    }

    /**
     * Get value from list of map entry nodes with key (name)
     * @param allMapEntryList : list of map entry nodes
     * @param name : name of the map
     * @return value of the map
     */
    String getValueFromMapEntryList(List allMapEntryList, String name) {

        for (TreeNodeWithProperties mapEntryTnwp : allMapEntryList) {
            TreeNodeWithProperties firstTnwp = mapEntryTnwp.getFirstChild()
            TreeNodeWithProperties lastTnwp = mapEntryTnwp.getLastChild()
            if (getTnwpText(firstTnwp).equals(name)) {
                return getTnwpText(lastTnwp)
            }
        }
        return null
    }
    ...
    /**
     * Get name, type, title from input
     * @param nodeTnwp : input node
     * @return name, type, title
     */
    List getInformFromInput(TreeNodeWithProperties nodeTnwp) {
        String inputNameStr = null
        String inputTypeStr = null
        String inputTitleStr = null
        ...
        if (getTnwpString(childArguList[0]) in ["MapExpression", "NamedArgumentListExpression"]) {
            ...
            inputNameStr = getValueFromMapEntryList(mapEntryList, "name")
            inputTypeStr = getValueFromMapEntryList(mapEntryList, "type")
            inputTitleStr = getValueFromMapEntryList(mapEntryList, "title")

            if (inputTitleStr == null) {
                inputTitleStr = ""
            }

            if (childArguList.size() == 2) {
                inputNameStr = getTnwpText(childArguList[1])
            } else if (childArguList.size() > 2) {
                inputNameStr = getTnwpText(childArguList[1])
                inputTypeStr = getTnwpText(childArguList[2])
            }
            ...
            inputTitleStr = ""
            if (childArguList.size() == 1) {
                // TODO
                String varTextStr = getTnwpText(childArguList[0])
                DefaultMutableTreeNode currDmtn = expressionToStatement(childArguList[0])
                while (currDmtn != null) {
                    ...
                    if (prevDmtn == null) {
                        ...
                        if (getTnwpString(prevDmtn) in ["ExpressionStatement - BinaryExpression", "ExpressionStatement - DeclarationExpression"]) {
                            ...
                            if (leftTextStr.equals(varTextStr)) {
                                List mapEntryList = rightTnwp.children().toList()
                                inputNameStr = getValueFromMapEntryList(mapEntryList, "name")
                                inputTypeStr = getValueFromMapEntryList(mapEntryList, "type")
                                inputTitleStr = getValueFromMapEntryList(mapEntryList, "title")
                                break
                ...
                if (inputNameStr == null) {
                    inputNameStr = varTextStr
                    inputTypeStr = ""
                }
            } else if (childArguList.size() == 2) {
                inputNameStr = getTnwpText(childArguList[0])
                inputTypeStr = getTnwpText(childArguList[1])
            } else if (childArguList.size() > 2) {
                inputNameStr = getTnwpText(childArguList[0])
                inputTypeStr = getTnwpText(childArguList[1])
                inputTitleStr = getTnwpText(childArguList[2])
            }
        ...
        return [inputNameStr, inputTypeStr, inputTitleStr]
    }

    /**
     * Get section string from input node
     * @param nodeTnwp : input node
     * @return section string
     */
    String getSectionFromInput(TreeNodeWithProperties nodeTnwp) {
        String sectionStr = ""
        DefaultMutableTreeNode sectionTnwp = nodeTnwp
        while (sectionTnwp != null && !getTnwpString(sectionTnwp).startsWith("MethodCall - this.section")) {
            sectionTnwp = sectionTnwp.getParent()
        }
        if (sectionTnwp == null) {
            return sectionStr
        }

        TreeNodeWithProperties argumentTnwp = sectionTnwp.getChildAt(2)
        List childArguList = argumentTnwp.children().toList()
        if (getTnwpString(childArguList[0]) in ["MapExpression", "NamedArgumentListExpression"]) {
            List mapEntryList = childArguList[0].children().toList()

            sectionStr = getValueFromMapEntryList(mapEntryList, "title")
            if (sectionStr == null) {
                if (childArguList.size() >= 2 && getTnwpString(childArguList[1]).startsWith("Constant")) {
                    sectionStr = getTnwpText(childArguList[1])
                } else {
                    sectionStr = ""
                }
            }
        } else if (getTnwpString(childArguList[0]) in ["ClosureExpression"]) {
            sectionStr = ""
        } else {
            sectionStr = getTnwpText(childArguList[0])
        }
    ...
    /**
     * Replace the method call node in the closure node. e.g. it.action in test.each{it.action()} turn into test.action
     * @return [TnwpNode:text], eg. it.action node : test.action
     */
    Map getClosureTnwpMap() {
        Map closureTnwpMap = [:]
        for (TreeNodeWithProperties tempTnwp : allClosureList) {
            String closureItStr = "it"
            TreeNodeWithProperties closureItTwnp = tempTnwp.getFirstChild()
            if (getTnwpString(closureItTwnp).startsWith("Parameter")) {
                closureItStr = getTnwpName(closureItTwnp)
            }

            List nodeEnumList = tempTnwp.depthFirstEnumeration().toList()
            List closureMethodCallList = getAllTnwpNodesList(nodeEnumList, "MethodCall")

            DefaultMutableTreeNode parentMethodCallTnwp = tempTnwp
            while (parentMethodCallTnwp != null && !(getTnwpString(parentMethodCallTnwp).startsWith("MethodCall"))) {
                parentMethodCallTnwp = parentMethodCallTnwp.getParent()
            }
            if (parentMethodCallTnwp == null) {
                continue
            }

            String parentRecvStr = getTnwpText(parentMethodCallTnwp.getFirstChild())
            for (TreeNodeWithProperties tempNode : closureMethodCallList) {
                String tempStr = getTnwpText(tempNode)
                List tempList = tempStr.split("\\.")
                if (tempList[0] - "?" == closureItStr) {
                    tempList[0] = parentRecvStr
                }
                closureTnwpMap[tempNode] = [parentRecvStr, tempList.join(".")]
            }

        }

        return closureTnwpMap
    }
