...
def format_2d_array(arr: np.ndarray) -> str:
    if arr.ndim != 2:
        raise ValueError("Only 2D arrays are supported.")
    return "{" + ",".join("{" + ",".join(map(str, row)) + "}" for row in arr) + "}"


def format_nd_array(arr, float_format="{:.2f}"):
    if isinstance(arr, np.ndarray):
        ...
        return "{" + ",".join(format_nd_array(sub, float_format) for sub in arr) + "}"
    elif isinstance(arr, (list, tuple)):
        return (
            "{"
            + ",".join(format_nd_array(np.array(sub), float_format) for sub in arr)
            + "}"
        )
    else:
        if isinstance(arr, float) and math.isnan(arr):
            arr = 0.0
        return float_format.format(arr) if isinstance(arr, float) else str(arr)


def make_table(xdata, ydata):
