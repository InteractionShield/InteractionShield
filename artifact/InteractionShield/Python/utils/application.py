...
from Python.utils.enums import AppTypeEnum, MethodTypeEnum, TcaEnum
...
class ApplicationClass:
    ...
    def __init__(
        self, name_s: str, detail_d: dict, filetype_u: AppTypeEnum, device_d: dict
    ):
        """
        ...
        self.vardevs: dict = self.get_variables_devices()

    def ifttt_process(self, detail_d):
        var4_s: str = detail_d["description"]
        self.description = var4_s
    ...
    def get_if_triggers_actions(self, detail_d, var4_s, var5_s):
        def get_if_value(if_id_s, NORMALIZE_VALUE_D):
            for key_s, values_l in NORMALIZE_VALUE_D.items():
                for value_s in values_l:
                    if value_s in if_id_s:
                        return key_s
            return if_id_s
        ...
        for comp_s in ["triggers", "actions"]:
            for item_d in detail_d[comp_s]:
                ...
                device_s = self.devmaps.get(capa_slug_s, "switch")
                if device_s in ["switch", "button"]:
                    device_s = random.choice(MULTI_SENSOR_DEVICES_L)

                self.inputs[capa_slug_s] = {
                    "device": device_s,
                    "type": item_d["service_name"],
                    "id": if_id_s,
                }

                state_s = get_if_value(if_id_s, NORMALIZE_VALUE_D)
    ...
    def get_if_tca_rules(self, triggers_l, actions_l):
        ...
        for action_tca_u in actions_l:
            ...
            for trigger_tca_u in triggers_l:
                rule_u = RuleClass(
                    self.category, trigger_tca_u, [], action_tca_u, self.appname
                )
                tca_rules_e.add(rule_u)
            if action_tca_u.type == TcaEnum.ACTUATOR:
                actuators_e.update(tca_rules_e)
            else:
                sinks_e.update(tca_rules_e)
        return sorted(actuators_e), sorted(sinks_e)

    def smartapp_process(self, detail_d):
        self.category = detail_d["categoryStr"].lower()
        ...
        self.inputs = self.parse_inputs(inputs_d)
    ...
    def get_sm_inputs(self, input_d: dict, var4_s: str, var5_s: str) -> dict:
        ...
        for varname_s, texts_l in input_d.items():
            ...
            if any(
                ...
                device_s = self.devmaps.get(varname_s, "switch")
                if device_s in ["switch", "button"] or capa_slug_s in [
                    "switch",
                    "button",
                ]:
                    device_s = random.choice(MULTI_SENSOR_DEVICES_L)
        ...
        for text_s, details_l in method_call_param_raw_d.items():
            ...
            if receiver_s == "this":
                if name_s in SUBSCRIBE_SM_METHOD_L:
                    ...
                    device_s = (
                        "time" + "|time." + inputs_d.get(arguments_l[0], arguments_l[0])
                    )
                ...
                elif name_s in SET_SM_METHOD_L:
                    device_s = "location"
    ...
    def convert_each_raw_action_to_tca_class(
        ...
        varname_s = method_call_u.receiver
        ...
        if method_call_u.receiver == "this":
            if method_call_u.name in SET_SM_METHOD_L:
                varname_s = "location"
                value_s = arguments_l[0]
                arguments_l = []
        elif value_s.startswith("set"):
            value_s = ""
            if len(arguments_l) > 0:
                value_s = arguments_l[0]
            arguments_l = []
    ...
    def get_sm_tca_rules(
        ...
        all_actuator_rules_e, all_sink_rules_e = set(), set()
        for method_call_text_s, raw_tca_rules_l in raw_tca_rules_d.items():
            for raw_rule_l in raw_tca_rules_l:
                ...
                if rule_u.action.type == TcaEnum.ACTUATOR:
                    all_actuator_rules_e.add(rule_u)
                elif rule_u.action.type in [
                    TcaEnum.SINK_MSG,
                    TcaEnum.SINK_HTTP,
                    TcaEnum.SINK_API,
                ]:
                    all_sink_rules_e.add(rule_u)

        return sorted(all_actuator_rules_e), sorted(all_sink_rules_e)
    ...
    def convert_each_raw_rule_to_tca_class(
        ...
        trigger_varname_s = trigger_method_u.arguments[0]
        ...
        if trigger_u is None:
            ...
            self.devsets.add(trigger_device_s)
        ...
        return RuleClass(self.category, trigger_u, conditions_l, action_u, self.appname)

    def openhab_process(self, detail_d: dict):
        self.description = detail_d["descriptionStr"]
    ...
    def get_op_inputs(self, input_d: dict) -> dict:
        ...
        for varname_s, texts_l in input_d.items():
            ...
            device_s = self.devmaps.get(varname_s, "switch")
            if device_s in ["switch", "button"] or capa_slug_s in ["switch", "button"]:
                device_s = random.choice(MULTI_SENSOR_DEVICES_L)
    ...
    def convert_op_each_raw_action_to_tca_class(
        ...
        if method_type_s == MethodTypeEnum.SINK_HTTP:
            ...
            if method_call_u.name in EVENT_OP_METHOD_L:
                if method_call_u.receiver == "this":
                    varname_s = arguments_l[0].strip('"').lower()
    ...
    def parse_inputs(self, inputs_d):
        parse_inputs_d: dict = {}
        for varname_s, inputs_u in inputs_d.items():
            parse_inputs_d[varname_s] = {
                "device": inputs_u.device,
                "type": inputs_u.type,
            }
        return parse_inputs_d

    def get_variables_devices(self):
        vardev = {}
        for rule_u in self.actuators_rules:
            for trigger_u in rule_u.triggers:
                vardev[trigger_u.varname] = trigger_u.device
            action_u = rule_u.action
            vardev[action_u.varname] = action_u.device
        return vardev
